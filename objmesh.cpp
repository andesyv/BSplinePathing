    ...
    while(std::getline(fileIn, oneLine))
    {
        ...
        if (oneWord == "f")
        {
            ...
            for(int i = 0; i < 3; i++)
            {
                ...
                if (segmentArray.size() > 1  && segmentArray[1] != "")              //second is uv
                ...
                {
                    Vertex tempVert(tempVertecies[index], tempNormals.size() > normal ? tempNormals[normal] : gsl::vec3{0.f, 0.f, 1.f}, gsl::Vector2D(0.0f, 0.0f));
                    mVertices.push_back(tempVert);
                }
